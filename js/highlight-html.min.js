...
tagClass:'tag',valueClass:'value',attrClass:'attr'},options);var input='<h1>Fill input!</h1>';var mode=settings.mode;if(settings.inputType==='selector'){input=document.querySelector(inp).outerHTML}else if(settings.inputType==='manual'){input=inp}else{throw new Error('inputType is not valid. Use \'selector\' (default) or \'manual\'.')}var tagClass=settings.tagClass,valueClass=settings.valueClass,attrClass=settings.attrClass;var tagBegin=/<\s?(\w*)\s?>/gi,tagEnd=/<\s?(\/\w*)\s?>/gi,classId=/<\s?(\w*)\s((?:class|id)\s?=\s?"[\w\s-]*>?")*/gi,hlStrings=new RegExp('(<\\s?span class\\s?=\\s?"'+tagClass+'"\\s?>[\\w-\\s]+<\\s?/span\\s?>\\s*)(class|id)\\s?=\\s?("[\\w-\\s]+")','ig'),hlStringsMultiple=new RegExp('(<\\s?span class\\s?=\\s?"'+valueClass+'"\\s?>"[\\w-\\s]+"<\\s?/span\\s?>\\s*)(class|id|data[-\\w]+)\\s?=\\s?("[\\w-\\s]+")','ig');var outputHTML=input.replace(tagBegin,'<$1>&lt;$1&gt;');//<div>
